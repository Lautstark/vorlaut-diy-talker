...
| The audio payload — whatever `seekToWavData()` walked past | [`wav_format.h`](../firmware/vorlaut/wav_format.h) and [`audio_format.ts`](../loader/src/audio_format.ts), six fixtures, four of them refusals. Since 2026-09-01 the file has a second form and the same treatment covers it — [ADR 0022](../adr/0022-recordings-may-travel-compressed.md), [`adpcm_format.h`](../firmware/vorlaut/adpcm_format.h) and [`audio_encode.ts`](../loader/src/audio_encode.ts), three more fixtures, and `test_adpcm.py` for the round trip, which requires the two decoders to agree byte for byte. |
