...
[`src/backend/cable.ts`](../src/backend/cable.ts) opens a Web Serial port;
[`tools/cable.js`](../tools/cable.js) speaks the wire; the release dialog
...
- **`compileDevice()`, `tiles.ts` and `tools/cable.js` belong to the page**, and
...
`tools/cable.js`, in one commit, the way the chunk-acknowledgement change landed
...
- **A build now needs a person twice.** The export needs a current build, so a
  Sammlung nobody has released still cannot be written down (ADR 0010), and the
  page needs somebody to open it with a cable in hand. Nothing happens to a
  talker that a person did not do in front of it, which is what was already true
