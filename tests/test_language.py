...
A handful of files are German on purpose and are skipped whole: the tables
that hold the German translations, the word lists that recognise German, this
file, and the changelog nobody writes by hand. Everything else is checked,
including the shell scripts and the Compose file - they are read while somebody
is setting the thing up, which is the same readership as the code.
"""
...
GERMAN_BY_DESIGN = {
    ...
    # Generated by release-please out of the commit subjects, and a subject
    # quoting the German string it changed is a good subject: `say "Zugriff
    # bestätigen" in the heading` says which heading. The rule here is that
    # *we* write English, and nobody writes this file - it is assembled from
    # history that cannot be edited afterwards without moving published tags.
    #
    # An entry in german.ALLOWED per offending line was the alternative, and it
    # does not hold: the changelog is rewritten in full at every release, so
    # that list would have to grow forever and would fail the release rather
    # than the commit that caused it. This appeared the day 0.1.0 was cut,
    # which is the first time the file existed at all.
    "CHANGELOG.md",
}
