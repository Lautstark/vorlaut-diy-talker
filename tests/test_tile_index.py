...
# Set before build is imported: it resolves the content directory at import
# time, and this test writes a cache. Without this it would be the
# developer's own.
...
def make_symbols() -> list[str]:
    ...
    build.SYMBOLS_DIR.mkdir(parents=True, exist_ok=True)
    names = []
    for i in range(SYMBOLS):
        name = f"sym{i:02d}.png"
        colour = (i * 9 % 256, (i * 37 + 20) % 256, (i * 71 + 90) % 256)
        Image.new("RGB", (16 + i, 16), colour).save(build.SYMBOLS_DIR / name)
        names.append(name)
    return names
    ...
    def write(offset: int) -> None:
        ...
        for round_number in range(ROUNDS):
            ...
            for i in range(len(names)):
                build.tile_bytes(names[(i * (offset + 1) + round_number) % len(names)])

    def read(number: int) -> None:
        start.wait()
        while not done.is_set():
            seen[number].append(len(build.load_tile_index()))
...
def main() -> int:
    with WORKSPACE:
        ...
        seen = hammer(names)
        index = build.load_tile_index()
        expected = {build.tile_fingerprint(name): name for name in names}

        check("every symbol has an entry", index == expected,
              f"{len(index)} of {len(expected)}")
        check("and a tile file to go with it",
              len(list(build.TILE_CACHE.glob("*.bin"))) == len(expected))
        ...
        check("nothing is left half written",
              not list(build.TILE_CACHE.glob("*.part")))
        ...
        build.prune_cache()
        after = build.load_tile_index()
        check("pruning keeps what the layout still names",
              all(build.tile_fingerprint(name) in after for name in keep),
              f"{sorted(after.values())}")
        check("and drops the rest",
              not any(build.tile_fingerprint(name) in after
                      for name in names[2:]))
        ...
        build._remember_build(layout)
        try:
            stored = json.loads(build.BUILD_STATE.read_text(encoding="utf-8"))
        except json.JSONDecodeError:
            stored = {}
        check("the build note is written whole",
              stored.get("fingerprint") == build.built_fingerprint(layout))
        check("and leaves nothing behind",
              not list(build.BUILD_STATE.parent.glob("*.part")))
