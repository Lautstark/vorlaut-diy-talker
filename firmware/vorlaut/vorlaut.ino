...
static void fetchContent() {
  ...
  // Where the computer is. There is one address, not one per network, so away
  // from home this usually points at nobody - and then the sync quietly does
  // not happen. A second network pays off in full only once the device finds
  // the editor by itself instead of being told where it is.
  const String host = settings.getString("host", "");
